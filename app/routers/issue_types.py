...
async def get_issue_type_queues(
    user: user_dependency,
    id: int,
    db: db_dependency
):
    ...
    return issue_type.queues


@router.post("/{id}/queues/{queue_id}")
async def add_issue_type_queue(
    user: user_dependency,
    id: int,
    queue_id: int,
    db: db_dependency
    ...
    if queue in issue_type.queues:
        raise HTTPException(
            status_code=400,
            detail="Queue is already associated with this Issue Type"
        )

    issue_type.queues.append(queue)

    db.commit()

    return {
        "message": "Queue added to Issue Type successfully"
    }

@router.delete("/{id}/queues/{queue_id}")
async def remove_issue_type_queue(
    user: user_dependency,
    id: int,
    queue_id: int,
    db: db_dependency
    ...
    if queue not in issue_type.queues:
        raise HTTPException(
            status_code=404,
            detail="Queue is not associated with this Issue Type"
        )

    issue_type.queues.remove(queue)

    db.commit()

    return {
        "message": "Queue removed from Issue Type successfully"
    }
...
async def get_issue_type_knowledge_base_notes(
    user: user_dependency,
    id: int,
    db: db_dependency
):
    ...
    return issue_type.knowledge_base_notes


@router.post("/{id}/knowledge-base-notes/{note_id}")
async def add_issue_type_knowledge_base_note(
    user: user_dependency,
    id: int,
    note_id: int,
    db: db_dependency
    ...
    if note in issue_type.knowledge_base_notes:
        raise HTTPException(
            status_code=400,
            detail="Knowledge Base Note is already associated with this Issue Type"
        )

    issue_type.knowledge_base_notes.append(note)

    db.commit()

    return {
        "message": "Knowledge Base Note added to Issue Type successfully"
    }


@router.delete("/{id}/knowledge-base-notes/{note_id}")
async def remove_issue_type_knowledge_base_note(
    user: user_dependency,
    id: int,
    note_id: int,
    db: db_dependency
    ...
    if note not in issue_type.knowledge_base_notes:
        raise HTTPException(
            status_code=404,
            detail="Knowledge Base Note is not associated with this Issue Type"
        )

    issue_type.knowledge_base_notes.remove(note)

    db.commit()

    return {
        "message": "Knowledge Base Note removed from Issue Type successfully"
    }
